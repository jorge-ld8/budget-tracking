...
import type {
  CreateTransactionAdminDto,
  CreateTransactionDto,
  TransactionQueryFiltersDto,
  UpdateTransactionAdminDto,
  UpdateTransactionDto,
} from "../types/dtos/transaction.dto.ts";
...
class TransactionController implements ITransactionController {
  ...
  async getAll(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const { items: transactions, totalDocuments } =
        await this.transactionService.getAll(userId, filters);
  ...
  async create(
    req: AuthenticatedRequest & { file: Express.MulterS3.File },
    res: Response,
    next: NextFunction
  ) {
    try {
  ...
  async delete(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const deletedId = await this.transactionService.delete(
        transactionId,
        userId
      );
      res
  ...
  async restore(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const transaction = await this.transactionService.restore(
        transactionId,
        userId
      );
      res
        .status(200)
        .json({ message: "Transaction restored successfully", transaction });
    } catch (error) {
      // Log or handle potential errors from the service method if it's not yet fixed
      console.error(
        "Error in TransactionController.restore potentially due to unfixed service method:",
        error
      );
      next(error);
    }
  ...
  async getDeleted(
    ...
    try {
      ...
      const deletedTransactions = await this.transactionService.getDeleted(
        userId
      );
      res.status(200).json({
        deletedTransactions,
      ...
      // Log or handle potential errors from the service method if it's not yet fixed
      console.error(
        "Error in TransactionController.getDeleted potentially due to unfixed service method:",
        error
      );
      next(error);
    }
  ...
  async getByAccount(
    ...
    try {
      ...
      const transactions = await this.transactionService.getByAccount(
        accountId,
        userId
      );
      res.status(200).json({
  ...
  async getByCategory(
    ...
    try {
      ...
      const transactions = await this.transactionService.getByCategory(
        categoryId,
        userId
      );
      res.status(200).json({
  ...
  async getAllAdmin(
    ...
    try {
      ...
      const filters: TransactionQueryFiltersDto = req.query;
      const { items: transactions, totalDocuments } =
        await this.transactionService.getAll(null, filters);
  ...
  async createAdmin(
    ...
    try {
      ...
      if (
        ...
        throw new BadRequestError(
          "Missing required fields for admin transaction creation."
        );
      }
      ...
      const transaction = await this.transactionService.createAdmin(
        transactionData,
        imageUrl
      );
      res.status(201).json({ transaction });
  ...
  async updateAdmin(
    ...
    try {
      ...
      const updatedTransaction = await this.transactionService.updateAdmin(
        transactionId,
        updateData
      );
      res.status(200).json({ transaction: updatedTransaction });
  ...
  async deleteAdmin(
    ...
    try {
      ...
      res
        .status(200)
        .json({
          message: "Transaction soft deleted successfully by admin",
          transactionId: deletedId,
        });
    } catch (error) {
      next(error);
  ...
  async restoreAdmin(
    ...
    try {
      ...
      res
        .status(200)
        .json({
          message: "Transaction restored successfully by admin",
          transaction,
        });
    } catch (error) {
      // Log or handle potential errors from the service method if it's not yet fixed
      console.error(
        "Error in TransactionController.restoreAdmin potentially due to unfixed service method:",
        error
      );
      next(error);
    }
  ...
  async getDeletedAdmin(
    ...
    try {
      ...
      // Log or handle potential errors from the service method if it's not yet fixed
      console.error(
        "Error in TransactionController.getDeletedAdmin potentially due to unfixed service method:",
        error
      );
      next(error);
    }
